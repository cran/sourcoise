...
#'
#' `sourcoise()` is used as a drop in replacement for `base::source()` but caches results on disk. Cache is persistant over sessions and can be shared through github.
#'
#'  `sourcoise()` looks like `base::source()`. However, there are some differences.
#'
#'  First, the script called in `sourcoise()` must end by a `return()` or by an object returned. Assignment made in the script won't be kept as `sourcoise()` is executed locally. Only explicitly returned object will be returned.
#'
#' So `soucoise()` is used by assigning its result to something (`aa <- sourcoise("mon_script.r)` or `sourcoise() |> ggplot() ...`).
#'
#' Unless specified otherwise with `wd` parameter, the working directory for the script execution is (temporarly) set to the dir in which is the script. That allows for simple access to companion files and permit to move the script and companion files to another dir or project.
#'
...
#'
#'  Third, if an error is triggered by the script, `sourcoise()` does not fail and return the error and attempts to retrun a cache, even invalid. However, if there is no (invalid or valid) cache, then an error is trigerred.
#'
...
#'
#' Whatever values takes the option `sourcoise.src_in`, if the file path starts with a `/`, then the source file will be interpreted from project root (if any). This is coherent whith naming convention in `quarto`. Otherwise, the document path wil be used firstly (if any, that is to say executed from quarto, rendering). Finally, working directory will be used. If everything fails, it will try to search in the project directory a corresponding file and will keep the closest from the calling point.
#'
#' Usually the fisrt call returns and caches the results. Results can be any R object and are serialized and saved using `qs2`. Subsequent calls, supposing none of cache invalidation conditions are true, are then very quick. No logging is used, data is fecteched from the cache and that's it. For standard size data, used in a table or a graph (< 1Mb roughly), return timing is under 10ms on a decent computer.
#'
#' `lapse` parameter is used for invalidation trigger 4. `lapse = "1 day"` ou `lapse="day"` for instance will trigger once a day the execution. `lapse = "3 days"` will do it every 72h. `hours`, `weeks`, `months`, `quarters` or `years` are understood time units. When `lapse` is defined for a script it will be used as long as a new lapse parameter is provided (such as "never" if you whish to stop expiration control)
#' More complex calendar instructions could be added, but `sourcoise_refesh()` provides a solution more general and easy to adapt to any use case, as to my knowledge, there is no general mechanism to be warned of data updates from websites.
#'
#' `track` is the trigger #3. It is simply a list of files (following path convention defined by `scr_in`, so either script dir of project dir as reference). If the files in the list are changed then the execution is triggered.
#' As for lapse, tracked files are accumulated over execution and a null track will not change the list of tracked files. To untrack use the `sourcoise_untrack()` function.
#' Track is done with a hash and it is impossible to have a cross plateform hash for excel files. Nevertheless, hash is done on text files with same results of different platforms.
#'
...
#' - `sourcoise.limit_mb` (integer) (default 50) individual cache data files size on disk limit. If file size is above the limit **no caching** occurs.
#'
#' @section Metadata:
#'
...
#' @param track (list) list of files which modification triggers cache invalidation and script execution. Default to `NULL`wich means no change in tracking.
#' @param lapse (character) duration over which cache is invalidated. Could be `NULL` (ie no change), `never` (default) `x hours`, `x days`, `x week`, `x months`, `x quarters`, `x years`.
...
#' @seealso [sourcoise_untrack()] [sourcoise_lapse()] [sourcoise_status()] [sourcoise_refresh()]
#' @export
...
sourcoise_ <- function(
  ...
  if(nrow(metas)>0 && any(metas$data_exist)) {
    gmeta <- metas |>
  ...
  if(prevent) {
    logger::log_fatal("{ctxt$src} prevented and no cache to return")
    rlang::abort(
      message = glue::glue("Execution of {ctxt$src} prevented and no cache to return"))
  }
  if(!prevent) {
    logger::log_fatal("{ctxt$src} failed and no cache to return")
    rlang::abort(
